...
const App: React.FC = () => {
  ...
  const calculateGPA = () => {
    const pastCredits = transcript.reduce((acc: number, c) => acc + c.credits, 0);
    const pastTGP = transcript.reduce((acc: number, c) => acc + c.tgp, 0);
    ...
    currentCourseCodes.forEach(code => {
      ...
      if (courseGrades.length > 0) {
        ...
        const totalMarks = courseGrades.reduce((acc: number, g) => acc + g.score, 0);
            ...
            <div className="bg-white dark:bg-[#2D1217] rounded-[2.5rem] p-4 sm:p-10 border border-rose-100 dark:border-rose-900/30 shadow-sm overflow-hidden">
                 ...
                 <table className="w-full text-left text-sm">
                   ...
                   <tfoot className="bg-rose-50/30 dark:bg-rose-900/10">
                      <tr className="font-black text-rose-600">
                        ...
                        <td className="py-4 text-center">{transcript.reduce((acc: number, c) => acc + c.credits, 0).toFixed(1)}</td>
                        <td colSpan={2}></td>
                        <td className="py-4 text-center">{transcript.reduce((acc: number, c) => acc + c.tgp, 0).toFixed(2)}</td>
                      </tr>
                   </tfoot>
                 </table>
               </div>
               <div className="mt-8 p-8 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-[2.5rem] flex flex-col sm:flex-row justify-between items-center gap-6 shadow-2xl">
                 <div className="text-center sm:text-left">
                    ...
                    <h4 className="text-2xl font-black">Credits Completed: {transcript.reduce((acc: number, c) => acc + c.credits, 0).toFixed(1)}</h4>
                 </div>
                 ...
                 <div className="text-center sm:text-right">
                    ...
                    <h4 className="text-5xl font-black">{ (transcript.reduce((acc: number, c) => acc + c.tgp, 0) / (transcript.reduce((acc: number, c) => acc + c.credits, 0) || 1)).toFixed(3) }</h4>
                 </div>
               </div>
            </div>
                ...
                <div className="max-w-md w-full bg-rose-50 dark:bg-rose-900/10 p-10 rounded-[3rem] border border-rose-100 dark:border-rose-900/30 text-center">
                   ...
                   <div className="mt-8 pt-8 border-t border-rose-100 dark:border-rose-900/20">
                     <div className="flex justify-between text-[11px] font-black text-slate-400 uppercase tracking-widest">
                       ...
                       <span className="text-rose-500 font-black">{(transcript.reduce((acc: number, c: TranscriptCourse) => acc + c.credits, 0) + Array.from(new Set(grades.map(g => g.courseCode))).reduce((acc: number, code) => {
